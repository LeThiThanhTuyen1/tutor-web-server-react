...
import { fadeIn, staggerContainer } from "../../ui/animation";
...
// Status styles with Vietnamese translations
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      if (response.succeeded && response.data) {
        ...
        toast({
          title: "Lỗi",
          description: response.message || "Không thể tải danh sách khóa học",
          variant: "destructive",
        });
      }
    } catch (err) {
    ...
    async (courseId: number) => {
      ...
      try {
        ...
        if (response.succeeded) {
          ...
          toast({
            title: "Lỗi",
            description: response.message || "Không thể đăng ký khóa học",
            variant: "destructive",
          });
        }
      } catch (error) {
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      ...
      if (response.succeeded) {
        toast({
          title: "Thành công",
          description: "Các khóa học đã được xóa thành công",
          variant: "success",
        });
        setSelectedCourses([]);
        fetchCourses();
        ...
        toast({
          title: "Lỗi",
          description: response.message || "Không thể xóa các khóa học",
          variant: "destructive",
        });
      }
    } catch (err) {
      toast({
        title: "Lỗi",
        description: "Không thể xóa các khóa học",
        variant: "destructive",
      });
    } finally {
  ...
  const EmptyState = useMemo(
    () => (
      <div className="text-center py-12">
        ...
        >
          <Book className="h-12 w-12 mx-auto text-indigo-600 dark:text-indigo-400 mb-4" aria-hidden="true" />
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
            Không tìm thấy khóa học
          </h3>
          <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6">
            Chúng tôi không tìm thấy khóa học nào phù hợp với tiêu chí của bạn. Hãy thử điều chỉnh bộ lọc hoặc từ khóa tìm kiếm.
          </p>
          <Button
            variant="outline"
                    ...
                    {statusOptions.map((status) => (
                      <div key={status} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          aria-label={`Lọc theo trạng thái ${translateStatus(status)}`}
                        />
                        <label
