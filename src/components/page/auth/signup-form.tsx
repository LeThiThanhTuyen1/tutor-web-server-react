...
export default function SignupForm({ onSwitchForm }: SignupFormProps) {
  ...
  const handleSendVerificationCode = async () => {
    ...
    try {
      ...
      if (response.succeeded) {
        setErrors("Làm ơn hãy kiểm tra email của bạn.");
        setIsVerified(true);
      } else {
        setErrors(response.message || "Có lỗi không mong muốn xảy ra.");
      }
    } catch (err) {
      setErrors("Có lỗi xảy ra. Hãy thử lại.");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (response.succeeded) {
      ...
      setErrors(response.message || "Mã xác thực không hợp lệ");
    }
    setIsLoading(false);
  };
    ...
    <div className="w-full">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-2">
          Hãy tham gia cộng đồng học tập của chúng tôi.
        </p>
      </div>
